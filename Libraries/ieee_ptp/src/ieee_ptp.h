...
// each port will require 4 connections plus ground (but everything should be
// common grounded)
struct ptp_port {
    ptp_port(uint8_t tx, uint8_t rx, uint8_t ms, uint8_t ss)
        : m_tx(tx), m_rx(rx), m_master_signal(ms), m_slave_signal(ss){};
    uint8_t m_tx;
    uint8_t m_rx;
...
class ptp {
   ...
   public:
    ptp(bool master, uint16_t f = 10, uint16_t microseconds = 100)
        : m_master(master), m_freq(f), m_ms(microseconds), num_ports(0) {
        if (m_master) {
    ...
    int attatch_ptp_port(uint8_t tx_pin, uint8_t rx_pin, uint8_t m_out,
                         uint8_t s_out);

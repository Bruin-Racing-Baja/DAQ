...
int ptp::attatch_ptp_port(uint8_t tx_pin, uint8_t rx_pin, uint8_t m_out, uint8_t s_out)
{
...
{
    //sets frequncy, will not allow to go higher than 10, per protocol regulations
    m_freq = std::min(f,10);
    return;
}
...
void ptp::synchronize_m()
{
    ...
    digitalWriteFast(m_i_pin,HIGH); //tell other micrcocontrollers its time to execute synchronization
    //recieve serial data

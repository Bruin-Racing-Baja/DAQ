...
LINTS::LINTS(uint8_t pin, bool master, uint16_t period) {
    ...
    m_master = master;
    m_recordings =
        new uint16_t[16];  // might not need to be dynamically allocated
    m_length = 0;
    m_period = period;
    m_state = 0;
}
LINTS::~LINTS() { delete[] m_recordings; }
...
void LINTS::begin() {
    if (m_master) {
        ...
        // if slave, set up interrupt on pin to call record function and start
        // timer
...
void LINTS::m_record() {
    ...
    m_length++;  // do at end so ,minimize time between storing time and
                 // starting
    return;
}